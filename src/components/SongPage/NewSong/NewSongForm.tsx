...
export const NewSongForm = ({ toggleIsEditing }: NewSongFormProps) => {
  ...
  const searchSong = (e: FormEvent<HTMLFormElement>) => {
    setSearching(true)
    e.preventDefault()
  ...
  const saveSong = (result: SpotifyTrack) => {
    const newSong = {
      ...
      <ModalContainer>
        ...
        <SearchForm onSubmit={searchSong}>
          ...
          <SearchInput
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => setArtistQuery(e.target.value)}
            value={artistQuery}
          />
        ...
        {results && results.length === 0 && !searching && titleQuery.length > 3 && (
