...
const Upvote = ({ changeVote, upvoted }: UpvoteProps) => {
  return (
    <VoteButton
      onClick={() => changeVote()}
      $upvoted={upvoted}
      aria-label={upvoted ? "Remove vote" : "Vote for this song"}
      title={upvoted ? "Remove vote" : "Vote for this song"}
    >
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 169 158">
...
const VoteButton = styled.button<{ $upvoted: boolean }>`
  background: none;
  border: none;
  cursor: pointer;
  padding: 8px;
  ...
  svg {
    ...
    height: 28px;
    fill: ${(props) =>
      props.$upvoted ? "var(--accent)" : "var(--text-secondary)"};
